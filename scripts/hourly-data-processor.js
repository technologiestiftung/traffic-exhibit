...
function processData() {
  return new Promise((resolve, reject) => {
    ...
    process.on("close", (code) => {
      if (code === 0) {
        console.log(
          `[${new Date().toISOString()}] Data processing completed successfully`
        );
        resolve();
      } else {
        console.error(
          `[${new Date().toISOString()}] Data processing failed with code ${code}`
        );
    ...
    process.on("error", (error) => {
      console.error(
        `[${new Date().toISOString()}] Error running data processing:`,
        error
      );
      reject(error);
    });
...
async function scheduleHourlyProcessing() {
  console.log(
    `[${new Date().toISOString()}] Starting hourly data processing scheduler...`
  );
  ...
  console.log(
    `[${new Date().toISOString()}] Scheduler started. Data will be processed every hour.`
  );
}
...
scheduleHourlyProcessing().catch((error) => {
  console.error(
    `[${new Date().toISOString()}] Failed to start scheduler:`,
    error
  );
  process.exit(1);
});
